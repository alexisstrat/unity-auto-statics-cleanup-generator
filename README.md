...
A backport of Unity 6.5's built-in `[AutoStaticsCleanup]` source generator to **Unity 6.0–6.4**. The emitted code, the attribute names, the nested-class layout, and the runtime base class all match Unity's own implementation; the goal is that source written against this generator compiles unchanged on Unity 6.5+ once the analyzer is removed.

Static fields, properties, and events marked `[AutoStaticsCleanup]` are reset on every Editor play-mode transition.
...
Drop these four files anywhere in `Assets/`. <br>**Namespaces for attributes and abstract class must be exactly as defined below**
...
`PlayModeScopeAutoCleanupRegistrar.cs` — editor-only central hook.
```csharp
